...
class IntentClassifier(BaseAnalyzer):
    ...
    # Intent patterns — raw strings
    _INTENT_PATTERN_STRS: dict[QueryIntent, list[str]] = {
    ...
    INTENT_PATTERNS: dict[QueryIntent, list[re.Pattern]] = {
        ...
        for intent, patterns in _INTENT_PATTERN_STRS.items()
    }
    ...
    def analyze(self, query: str) -> dict[str, Any]:
        ...
        if sorted_intents and sorted_intents[0][1] > 0:
            ...
            secondary_intents = [intent for intent, score in sorted_intents[1:] if score > 0]
    ...
    def analyze(self, query: str) -> dict[str, Any]:
        ...
        for match in self._CAP_PHRASE_RE.finditer(query):
            ...
            already_captured = any(
                e.start <= match.start() < e.end or e.start < match.end() <= e.end for e in entities
            )
...
class QueryUnderstandingEngine:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if stats["total_queries"] > 0:
            stats["avg_analysis_time_ms"] = stats["total_analysis_time_ms"] / stats["total_queries"]
        else:
