...
class Sentence:
    ...
    def __post_init__(self) -> None:
        """Compute word count only when not explicitly provided by the caller."""
...
class TextRankScorer:
    ...
    def _build_similarity_matrix(self, sentences: List[Sentence]) -> np.ndarray:
        """Build word overlap similarity matrix."""
        n = len(sentences)
        matrix = np.zeros((n, n))

        # Tokenize sentences
        sentence_words = [set(s.text.lower().split()) for s in sentences]
...
class ExtractiveSummarizer:
    ...
    def summarize(
        ...
        # Select sentences
        selected = self._select_sentences(sentences, max_length, max_sentences)
    ...
    def _is_redundant(self, sentence: Sentence, selected: List[Sentence]) -> bool:
...
class MultiDocumentSummarizer:
    ...
    def summarize(
        ...
        selected = self._select_diverse(all_sentences, max_length, max_sentences, len(documents))
...
def get_summarizer(config: Optional[SummaryConfig] = None) -> DocumentSummarizer:
...
def summarize_for_query(
    ...
    Returns:
        Summary string
    """
    result = get_summarizer().summarize_for_query(text, query, max_length=max_length)
    return result.summary
