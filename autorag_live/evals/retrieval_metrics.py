...
class F1Metric(BaseMetric):
    ...
    def __init__(self, k: Optional[int] = None):
        ...
        """
        self.k = k
        self._precision = PrecisionMetric(k)
        self._recall = RecallMetric(k)
    ...
    def compute(
        ...
        **kwargs,
        ...
        """Compute F1 score."""
        precision = self._precision.compute(retrieved, relevant)
        recall = self._recall.compute(retrieved, relevant)
...
class NDCGMetric(BaseMetric):
    ...
    def compute(
        ...
        **kwargs,
        ...
        # Compute ideal DCG
        ideal_order = sorted(relevant, key=lambda d: relevance_scores.get(d, 0), reverse=True)
        if self.k:
...
class RetrievalMetrics:
    ...
    def evaluate(
        ...
        scores.num_relevant_retrieved = len(set(retrieved) & relevant)
    ...
    def evaluate_batch(
        ...
        for result in results:
            ...
            if qrels and result.query_id in qrels:
                ...
                relevant = {doc_id for doc_id, j in result.judgments.items() if j.is_relevant}
            else:
                continue
            ...
            if result.judgments:
                relevance_scores = {doc_id: j.score for doc_id, j in result.judgments.items()}
    ...
    def _average_scores(
        ...
        # @K metrics
        for k in self.k_values:
            avg.precision_at[k] = sum(s.precision_at.get(k, 0) for s in scores_list) / n
            avg.recall_at[k] = sum(s.recall_at.get(k, 0) for s in scores_list) / n
            avg.ndcg_at[k] = sum(s.ndcg_at.get(k, 0) for s in scores_list) / n
            avg.hit_rate_at[k] = sum(s.hit_rate_at.get(k, 0) for s in scores_list) / n

        # Totals
        avg.num_retrieved = sum(s.num_retrieved for s in scores_list)
        avg.num_relevant = sum(s.num_relevant for s in scores_list)
        avg.num_relevant_retrieved = sum(s.num_relevant_retrieved for s in scores_list)
