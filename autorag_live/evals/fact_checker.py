...
class ClaimExtractor:
    ...
    def _is_valid_claim(self, sentence: str) -> bool:
        ...
        if not self._WORD_RE.search(sentence):
            return False
...
class TextOverlapVerifier(BaseVerifier):
    ...
    def verify(
        ...
        for idx, source in enumerate(sources):
            ...
            # Combined score
            score = (1 - self.entity_weight) * overlap_score + self.entity_weight * entity_score
...
class FactChecker:
    ...
    def get_unsupported_claims(
        ...
        """
        result = self.verify(answer, sources)
        return [v.claim for v in result.verifications if v.status != VerificationStatus.SUPPORTED]
                ...
                if 0 <= idx < len(sources):
                    ...
                    if current_text.strip():
                        verification = self.fact_checker.verify(
                            current_text.strip(), [sources[idx]]
                        )
...
class ConsistencyChecker:
    ...
    def _check_numerical_contradiction(
        ...
        common_entities = set(e.lower() for e in claim1.entities) & set(
            e.lower() for e in claim2.entities
        )
