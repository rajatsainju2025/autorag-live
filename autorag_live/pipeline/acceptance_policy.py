...
class AcceptancePolicy:
    ...
    def evaluate_change(self, config_backup_paths: Dict[str, str], runs_dir: str = "runs") -> bool:
        ...
        logger.debug(
            f"Current run {current_run['run_id']}: {self.metric_key} = {current_metric:.4f}"
        )
        ...
        logger.debug(
            f"Best baseline: {self.metric_key} = {best_metric:.4f}, improvement = {improvement:.4f}"
        )
        ...
        if improvement >= self.threshold:
            ...
            logger.warning(
                f"❌ Changes REVERTED: {self.metric_key} change {improvement:.4f} < threshold {self.threshold}"
            )
            return False
