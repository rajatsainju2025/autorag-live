...
# Pre-compiled patterns for _clean_markdown
...
_MD_ITALIC_ASTERISK_RE = re.compile(r"\*([^*]+)\*")
_MD_ITALIC_UNDER_RE = re.compile(r"_([^_]+)_")
_MD_HR_RE = re.compile(r"^[-*_]{3,}$", re.MULTILINE)

# Pre-compiled patterns for _strip_html
...
class MarkdownParser(DocumentParser):
    ...
    def _clean_markdown(self, text: str) -> str:
        ...
        # Remove code blocks
        clean = _MD_CODE_BLOCK_RE.sub("", text)

        # Remove inline code
        clean = _MD_INLINE_CODE_RE.sub("", clean)

        # Remove links (keep text)
        clean = _MD_LINK_RE.sub(r"\1", clean)

        # Remove images
        clean = _MD_IMAGE_RE.sub("", clean)

        # Remove headings syntax
        clean = _MD_HEADING_RE.sub("", clean)

        # Remove emphasis
        clean = _MD_BOLD_ASTERISK_RE.sub(r"\1", clean)
        clean = _MD_BOLD_UNDER_RE.sub(r"\1", clean)
        clean = _MD_ITALIC_ASTERISK_RE.sub(r"\1", clean)
        clean = _MD_ITALIC_UNDER_RE.sub(r"\1", clean)

        # Remove horizontal rules
        clean = _MD_HR_RE.sub("", clean)
...
class HTMLParser(DocumentParser):
    ...
    def _strip_html(self, html: str) -> str:
        ...
        clean = _HTML_SCRIPT_RE.sub("", html)
        clean = _HTML_STYLE_RE.sub("", clean)
        ...
        # Clean whitespace
        clean = _HTML_WHITESPACE_RE.sub(" ", clean)
