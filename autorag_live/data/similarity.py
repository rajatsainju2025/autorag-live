...
class JaccardSimilarity(BaseSimilarityMetric):
    ...
    def prepare(self, doc: Document) -> Document:
        ...
        if self.use_shingles:
            doc.shingles = self.preprocessor.get_shingles(doc.content, self.shingle_size)
        else:
    ...
    def compute(
        ...
        if self.use_shingles:
            set1 = doc1.shingles or self.preprocessor.get_shingles(doc1.content, self.shingle_size)
            set2 = doc2.shingles or self.preprocessor.get_shingles(doc2.content, self.shingle_size)
        else:
...
class MinHashSimilarity(BaseSimilarityMetric):
    ...
    def __init__(
        ...
        random.seed(42)
        self._hash_params = [
            (random.randint(1, 2**31), random.randint(0, 2**31)) for _ in range(num_hashes)
        ]

    def _hash(self, value: str, a: int, b: int) -> int:
        """Apply a hash function."""
        h = int.from_bytes(hashlib.md5(value.encode()).digest(), "big")
        return (a * h + b) % (2**31 - 1)
