...
class InMemoryBackend(StorageBackend):
    ...
    def _apply_filter(
        ...
        # Filter by IDs — convert to set for O(1) membership
        if filter.ids:
            ids_set = set(filter.ids)
            result = [d for d in result if d.id in ids_set]

        # Filter by tags — convert to set for O(1) membership
        if filter.tags:
            tags_set = set(filter.tags)
            result = [d for d in result if any(t in tags_set for t in d.tags)]

        # Filter by sources — convert to set for O(1) membership
        if filter.sources:
            sources_set = set(filter.sources)
            result = [d for d in result if d.source in sources_set]
        ...
        if filter.metadata_equals:
            result = [
                d
                for d in result
                if all(d.metadata.get(k) == v for k, v in filter.metadata_equals.items())
            ]
        ...
        if filter.content_contains:
            ...
            result = [d for d in result if search_term in d.content.lower()]
...
class SQLiteBackend(StorageBackend):
    ...
    def list(self, filter: Optional[QueryFilter] = None) -> List[Document]:
        ...
        if filter:
            if filter.tags:
                docs = [d for d in docs if any(t in d.tags for t in filter.tags)]
