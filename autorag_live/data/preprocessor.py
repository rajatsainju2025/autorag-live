...
class MarkdownCleaner(BasePreprocessingStep):
    ...
    _RE_LINK_TEXT_ONLY = re.compile(r"\[([^\]]+)\]\([^)]+\)")
    ...
    _RE_HR = re.compile(r"^[-*_]{3,}\s*$", flags=re.MULTILINE)
    _RE_UNORDERED_LIST = re.compile(r"^[\s]*[-*+]\s+", flags=re.MULTILINE)
    _RE_ORDERED_LIST = re.compile(r"^[\s]*\d+\.\s+", flags=re.MULTILINE)
    ...
    def process(self, text: str) -> str:
        ...
        if self.preserve_code_blocks:
            for i, match in enumerate(self._RE_CODE_BLOCK.finditer(text)):
        ...
        if self.preserve_links:
            text = self._RE_LINK_WITH_URL.sub(r"\1 (\2)", text)
        else:
            text = self._RE_LINK_TEXT_ONLY.sub(r"\1", text)

        # Remove emphasis
        text = self._RE_BOLD_STAR.sub(r"\1", text)  # Bold
        text = self._RE_ITALIC_STAR.sub(r"\1", text)  # Italic
        text = self._RE_BOLD_UNDER.sub(r"\1", text)  # Bold
        text = self._RE_ITALIC_UNDER.sub(r"\1", text)  # Italic
        ...
        # Remove list markers
        text = self._RE_UNORDERED_LIST.sub("", text)
        text = self._RE_ORDERED_LIST.sub("", text)
...
class ContentTypeDetector:
    ...
    def detect(self, text: str) -> ContentType:
        ...
        markdown_matches = sum(1 for p in markdown_patterns if re.search(p, text, re.MULTILINE))
