...
class FeatureExtractor:
    ...
    COMPARISON_TERMS = {
        "vs", "versus", "compared", "compare", "comparing", "comparison",
        "difference", "differences", "different", "differ",
    ...
    # Temporal indicators
    TEMPORAL_TERMS = {
        "when", "time", "date", "year", "month", "day", "week",
        "before", "after", "during", "since", "until",
        "today", "tomorrow", "yesterday",
        "recent", "recently", "latest", "newest",
        "history", "historical", "past", "future",
    ...
    # Spatial indicators
    SPATIAL_TERMS = {
        "where", "location", "place", "address", "city", "country",
        "near", "nearby", "closest", "distance",
    ...
    def __init__(self):
        ...
        self._listing_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.LISTING_PATTERNS
        ]
    ...
    def extract(self, query: str) -> IntentFeatures:
        ...
        features.comparison_terms = [
            t for t in tokens if t in self.COMPARISON_TERMS
        ]
        
        # Find temporal terms
        features.temporal_terms = [
            t for t in tokens if t in self.TEMPORAL_TERMS
        ]
        
        # Check listing patterns
        features.listing_patterns = any(
            p.search(normalized) for p in self._listing_patterns
        )
...
class RuleBasedClassifier:
    ...
    def classify(self, query: str) -> Dict[IntentType, float]:
        ...
        # Spatial detection
        spatial_terms = [
            t for t in features.tokens 
            if t in self.extractor.SPATIAL_TERMS
        ]
        if spatial_terms:
        ...
        if max_score > 0:
            for intent in scores:
                scores[intent] = min(1.0, scores[intent] / max_score if max_score > 1.0 else scores[intent])
...
class KeywordClassifier:
    ...
    INTENT_KEYWORDS: Dict[IntentType, Dict[str, float]] = {
        IntentType.FACTUAL: {
            "what": 0.3, "who": 0.4, "where": 0.3, "when": 0.3,
            "is": 0.2, "are": 0.2, "was": 0.2, "were": 0.2,
            "name": 0.3, "number": 0.3, "count": 0.3,
            "fact": 0.4, "true": 0.3, "false": 0.3,
        },
        IntentType.ANALYTICAL: {
            "analyze": 0.5, "analysis": 0.5, "evaluate": 0.5,
            "assess": 0.4, "examine": 0.4, "investigate": 0.4,
            "why": 0.3, "impact": 0.4, "effect": 0.4,
            "trend": 0.4, "pattern": 0.4, "correlation": 0.5,
        },
        IntentType.COMPARISON: {
            "vs": 0.6, "versus": 0.6, "compare": 0.6, "comparison": 0.6,
            "difference": 0.5, "differences": 0.5, "different": 0.4,
            "similar": 0.4, "similarity": 0.5, "better": 0.4, "worse": 0.4,
            "prefer": 0.4, "alternative": 0.4, "or": 0.2,
        },
        IntentType.OPINION: {
            "should": 0.4, "recommend": 0.5, "suggestion": 0.5,
            "advice": 0.5, "opinion": 0.5, "think": 0.3,
            "best": 0.3, "prefer": 0.4, "worth": 0.4,
            "favorite": 0.4, "ideal": 0.4,
        },
        IntentType.INSTRUCTION: {
            "how": 0.4, "steps": 0.5, "step": 0.5, "guide": 0.5,
            "tutorial": 0.5, "instructions": 0.5, "process": 0.4,
            "create": 0.4, "build": 0.4, "make": 0.3, "setup": 0.5,
            "install": 0.5, "configure": 0.5, "implement": 0.4,
        },
        IntentType.DEFINITION: {
            "define": 0.6, "definition": 0.6, "meaning": 0.5,
            "explain": 0.4, "describe": 0.4, "what": 0.2,
            "concept": 0.4, "term": 0.4, "terminology": 0.5,
        },
        IntentType.TEMPORAL: {
            "when": 0.5, "time": 0.4, "date": 0.5, "year": 0.4,
            "month": 0.4, "day": 0.4, "history": 0.4,
            "before": 0.3, "after": 0.3, "during": 0.3,
            "recent": 0.4, "latest": 0.4, "current": 0.3,
        },
        IntentType.SPATIAL: {
            "where": 0.5, "location": 0.5, "place": 0.4,
            "address": 0.5, "city": 0.4, "country": 0.4,
            "near": 0.4, "distance": 0.4, "region": 0.4,
        },
        IntentType.CAUSAL: {
            "why": 0.5, "cause": 0.5, "reason": 0.5,
            "because": 0.4, "result": 0.4, "lead": 0.3,
            "effect": 0.4, "consequence": 0.5, "outcome": 0.4,
        },
        IntentType.LISTING: {
            "list": 0.6, "show": 0.3, "all": 0.3, "examples": 0.5,
            "types": 0.4, "kinds": 0.4, "categories": 0.4,
            "top": 0.4, "best": 0.3, "options": 0.4,
        },
    }
...
class IntentClassifier:
    ...
    def classify(self, query: str) -> IntentResult:
        ...
        aggregated_scores: Dict[IntentType, float] = {
        ...
        intents_above_threshold = [
            (intent, score) for intent, score in sorted_intents
    ...
    def _determine_query_type(
        self, query: str, features: IntentFeatures
    ) -> QueryType:
        ...
        command_starters = {
            "show", "list", "give", "find", "search", "get",
            "tell", "explain", "describe", "define", "compare",
        }
