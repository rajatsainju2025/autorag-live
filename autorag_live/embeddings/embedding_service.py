...
class EmbeddingCache:
    ...
    def put(self, text: str, model: str, provider: str, embedding: List[float]) -> None:
...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    """OpenAI embedding provider with connection pooling."""
            ...
            with self._client_lock:
                if self._client is None:
                    try:
                        ...
                        raise ImportError("openai and httpx required: pip install openai httpx")
...
class EmbeddingService:
        ...
        **kwargs: Any,
        ...
        for attempt in range(self.config.max_retries):
            ...
            try:
                ...
                logger.warning(
                    f"Embedding failed (attempt {attempt + 1}): {e}. " f"Retrying in {delay:.2f}s"
                )
                time.sleep(delay)
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_latency_ms": (
                self._total_latency / self._total_requests if self._total_requests > 0 else 0
            ),
