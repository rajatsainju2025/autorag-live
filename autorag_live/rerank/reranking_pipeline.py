...
class BM25Reranker(BaseReranker[str]):
    ...
    def rerank(
        ...
        bm25_scores = [
            self._compute_bm25(
                query_terms, doc_terms, avg_doc_len, doc_freqs, len(items)
            )
            for doc_terms in doc_terms_list
        ]
...
class DiversityReranker(BaseReranker[str]):
    ...
    def rerank(
        ...
        relevance_scores = [
            self._cosine_similarity(query_emb, item_emb)
            for item_emb in item_embs
        ]
...
class ContextualReranker(BaseReranker[str]):
    ...
    def rerank(
        ...
        scored_items = list(zip(
            items, combined_scores, original_scores, range(len(items))
        ))
        scored_items.sort(key=lambda x: x[1], reverse=True)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if stats["total_reranks"] > 0:
            stats["avg_latency_ms"] = (
                stats["total_latency_ms"] / stats["total_reranks"]
            )
            stats["avg_items_per_rerank"] = (
                stats["total_items_processed"] / stats["total_reranks"]
            )
        else:
...
def create_default_pipeline(
    ...
    if use_cross_encoder:
        pipeline.add_stage(CrossEncoderReranker(RerankerConfig(
            reranker_type=RerankerType.CROSS_ENCODER,
            top_k=20,
        )))
    
    if use_diversity:
        pipeline.add_stage(DiversityReranker(
            RerankerConfig(
