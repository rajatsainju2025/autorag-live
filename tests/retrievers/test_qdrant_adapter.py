...
def mock_qdrant_imports():
    ...
    with patch("autorag_live.retrievers.qdrant_adapter.QDRANT_AVAILABLE", True), patch(
        "autorag_live.retrievers.qdrant_adapter.QdrantClient"
    ) as mock_client_class, patch(
        "autorag_live.retrievers.qdrant_adapter.Distance"
    ) as mock_distance, patch(
        "autorag_live.retrievers.qdrant_adapter.VectorParams"
...
class TestQdrantRetriever:
    ...
    def test_qdrant_retriever_initialization(self, mock_qdrant_imports):
        ...
        retriever = QdrantRetriever(
            model_name="test-model", collection_name="test_collection", host="localhost", port=6333
        )
    ...
    def test_build_index(self, mock_qdrant_imports, mock_sentence_transformer, sample_docs):
        ...
        mock_client.get_collection.return_value = MagicMock(
            vectors_count=0, config=MagicMock(params=MagicMock(vectors=MagicMock(size=384)))
        )
    ...
    def test_save_and_load(self, mock_qdrant_client, mock_distance, tmp_path):
        ...
        retriever = QdrantRetriever(collection_name="test_collection", host="test.host", port=6333)
    ...
    def test_build_index_creates_collection(
        self, mock_qdrant_client, mock_distance, mock_vector_params
    ):
    ...
    def test_save_and_load_config(self, mock_qdrant_client, mock_distance):
        ...
        retriever = QdrantRetriever(
            model_name="test-model", collection_name="test_collection", distance_metric="cosine"
        )
